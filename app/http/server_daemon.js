...
const logStream = fs.createWriteStream( "./log/http_server.log" );

function log( msg ) {
	logStream.write( msg + "\n" );
}

log( "Starting Log ["+ new Date() +"]\n" );
...
console.log( "process.env.DB_HOST b", process.env.DB_HOST )
...
if ( process.env.NODE_ENV == "production" ) {
	app.get( "/bundle.js", ( request, response ) => {
		response.sendFile( process.cwd() + "/node_modules/innoutchallenge_frontend/build/bundle.js" );
	});
	app.get( "/*", checkAuthenticationView, ( request, response ) => {
		response.sendFile( process.cwd() + "/node_modules/innoutchallenge_frontend/build/index.html" );
	});
}
